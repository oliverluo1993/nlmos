...
These ``orthogonalization tails'' complicate the transferability of chemically relevant electronic-structure information from one system to another and, more importantly, reduce orbital locality making orbital-based local correlation methods less computationally efficient.
%
...
It has been found that NLMOs are indeed about $10-30 \%$ more localized than OLMOs if measured by the value of the Boys-Foster (RZK: check which functional is used) functional~\cite{feng2004An_efficient, liu2000nonorthogonal}. 
...
%% RZK: introduction review has stopped here. Intro belo needs further modifications
...
The latter problem is particularly widespread and will be referred to as the collapse problem. 
To overcome this problem, Yang and co-workers~\cite{feng2004An_efficient, cui2010efficient} have developed a method to construct NLMOs by fixing the centers of NLMOs during the minimization of the localization functional. 
The positions of the centers are predefined using the corresponding OLMOs~\cite{feng2004An_efficient} or obtained good understanding of bonding patterns in the systems (i.e.``chemical intuition'')~\cite{cui2010efficient}.
