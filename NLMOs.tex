...
The procedure for tuning $c_P$ is shown as the outer loop of the algorithm in Figure~\ref{fig:cg}. Its only required input is $\text{D}_{\text{min}}$. % and optional parameter $\alpha$ that can improve efficiency of the optimization. %RZZK
...
The localization procedure was implemented in the CP2K software package~\cite{cp2kgeneral}. 
Initial delocalized orbitals All calculations were preformed using DFT as implemented in the CP2K software package~\cite{cp2kgeneral}.
