...
This scheme is based on maximizing the Mulliken charge~\cite{mulliken1955electronic} or L{\"o}wdin charge~\cite{lowdin1950non} of each orbital.
...
The final obtained LMOs listed in the Figure~\ref{fig:pipek} are no longer restricted to NLMOs by increasing the final determinant cutoff upto 0.99.
According to the first row of Figure~\ref{fig:pipek}, our results maintain the $\sigma$ and $\pi$ separation in OLMOs, which are illustrated the $\pi$ bond in benzene molecule and perfectly separated $\sigma$ and $\pi$ bond in propenol molecule.
With the final determinant decreasing and the LMOs becoming more and more nonorthogonal, we surprisingly noticed that the property of $\sigma$ and $\pi$ separation of Pipek-Mezey scheme is no longer satisfied.
The NLMO of $\pi$ bond becomes more compact and tend to generate the $\tau$ bond with $\sigma$ bond in the benzene molecule.
The first two column of Figure~\ref{fig:pipek} illustrates the comparison between Boys-Foster and Pipek-Mezey localizaiton scheme.
Even though the failure of $\sigma$-$\pi$ separation when generating NLMOs with Pipek-Mezey scheme, we still notice the generated NLMOs with Pipek-Mezey scheme are better representative of bonding pattern and more localized compared with those with Boys-Foster scheme.
...
In the propenol molecule, the $\pi$ and $\sigma$ bonds are tend to generate the $\tau$ and $\tau'$ bonds, which is the almost same result as we obtained in the Boys-Foster scheme.
In the Pipek-Mezey localization scheme, the previous study has proved that the $\sigma$-$\pi$ separtion is always the stable solution compare with the mixed $\tau$ and $\tau'$ bonds for optimum population localized orbitals in Eq.~\ref{eq:pipek-tau}. The expressions of $\ket{\tau}$ and $\ket{tau'}$ of Pipek-Mezay NLMOs scheme (Eq.~\ref{eq:non-ortho-pipek}) are different from the OLMO scheme, which results in the $\pi$ and $\sigma$ separation is no longer more stable than the mixed solution.
So that the $\sigma$ and $\pi$ seapration is not hold in the nonorthogonal otbitals even based on Pipek-Mezey localization functional.
